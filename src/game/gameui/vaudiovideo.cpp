...
void AudioVideo::Activate()
{
	...
	if ( m_drpColorMode )
	{
		...
		{
			m_drpColorMode->SetCurrentSelection( "#GameUI_ColorMode_Television" );
		}
		else
		{
			m_drpColorMode->SetCurrentSelection( "#GameUI_ColorMode_LCD" );
		}
	...
	if ( m_drpSplitScreenDirection )
	{
		...
		if ( !bWidescreen )
		{
			...
			m_drpSplitScreenDirection->SetCurrentSelection( "#GameUI_SplitScreenDirection_Horizontal" );
		}
		else
		{
			...
			case 1:
				...
				break;
			case 2:
				m_drpSplitScreenDirection->SetCurrentSelection( "#GameUI_SplitScreenDirection_Vertical" );
				break;
			default:
				m_drpSplitScreenDirection->SetCurrentSelection( "#GameUI_SplitScreenDirection_Default" );
			}
		}
	...
	if ( m_drpGore )
	{
		...
		{
			m_drpGore->SetCurrentSelection( "#GameUI_Gore_Low" );
		}
		else
		{
			m_drpGore->SetCurrentSelection( "#GameUI_Gore_High" );
		}
...
{
	if ( !Q_stricmp( command, "#GameUI_ColorMode_Television" ) )
	{
		CGameUIConVarRef mat_monitorgamma_tv_enabled( "mat_monitorgamma_tv_enabled" );
		mat_monitorgamma_tv_enabled.SetValue( 1 );
		m_bDirtyVideoConfig = true;
	}
	else if ( !Q_stricmp( command, "#GameUI_ColorMode_LCD" ) )
	{
		CGameUIConVarRef mat_monitorgamma_tv_enabled( "mat_monitorgamma_tv_enabled" );
		mat_monitorgamma_tv_enabled.SetValue( 0 );
		m_bDirtyVideoConfig = true;
	}
	else if ( !Q_stricmp( command, "#GameUI_SplitScreenDirection_Default" ) )
	{
	...
	else if ( !Q_stricmp( command, "#GameUI_SplitScreenDirection_Horizontal" ) )
	{
		if ( m_drpSplitScreenDirection && m_drpSplitScreenDirection->IsEnabled() )
		{
			CGameUIConVarRef ss_splitmode( "ss_splitmode" );
	...
	else if ( !Q_stricmp( command, "#GameUI_SplitScreenDirection_Vertical" ) )
	{
		if ( m_drpSplitScreenDirection && m_drpSplitScreenDirection->IsEnabled() )
		{
			CGameUIConVarRef ss_splitmode( "ss_splitmode" );
	...
	else if ( !Q_stricmp( command, "#GameUI_Gore_High" ) )
	{
		CGameUIConVarRef z_wound_client_disabled( "z_wound_client_disabled" );
		z_wound_client_disabled.SetValue( 0 );
		m_bDirtyVideoConfig = true;
	}
	else if ( !Q_stricmp( command, "#GameUI_Gore_Low" ) )
	{
		CGameUIConVarRef z_wound_client_disabled( "z_wound_client_disabled" );
...
void AudioVideo::UpdateFooter()
{
	...
	if ( footer )
	{
		...
		footer->SetButtonText( FB_BBUTTON, "#GameUI_Controller_Done" );
	}
}
...
{
	BaseClass::DrawDialogBackground( "#GameUI_AudioVideo_Title", NULL, "#GameUI_AudioVideo_Desc", NULL );
}
