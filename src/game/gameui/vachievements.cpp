...
void AchievementListItem::SetGamerScore(int score)
{
	...
	g_pVGuiLocalize->ConstructString( buffer, sizeof( buffer ), g_pVGuiLocalize->FindSafe( "#GameUI_Gamerscore_Progress" ), 1, num );
...
BaseClass(parent, panelName)
{
	...
	m_LblCategory = new Label( this, "LblCategory", "#GameUI_Unattained" );
...
BaseClass(parent, panelName, false, true)
{
	...
	if ( IsX360() )
	{
		...
		const wchar_t *pwcTemplate = g_pVGuiLocalize->Find("#GameUI_My_Achievements_User");
...
void Achievements::Activate()
{
	...
	g_pVGuiLocalize->ConstructString( m_wAchievementsProgress, sizeof( m_wAchievementsProgress ), g_pVGuiLocalize->Find( "#GameUI_Achievement_Progress" ), 2, wNumAchieved, wTotalAchievements );
	...
	g_pVGuiLocalize->ConstructString( localizedGamerscoreProgress, sizeof( localizedGamerscoreProgress ), g_pVGuiLocalize->Find( "#GameUI_Gamerscore_Progress" ), 2, wGamerscore );
...
void Achievements::UpdateFooter()
{
	...
	if ( footer )
	{
		...
		footer->SetButtonText( FB_YBUTTON, m_bShowingAssets ? "#GameUI_ShowAchievements" : "#GameUI_ShowUnlockableAwards" );
	}
}
...
void Achievements::PaintBackground()
{
	if ( IsPC() )
	{
		//BaseClass::DrawDialogBackground( "#GameUI_My_Achievements", NULL, "#GameUI_My_Achievements_Desc", NULL );
...
*/
	...
	else
	{
		...
		{
			BaseClass::DrawDialogBackground( "#GameUI_My_Achievements", NULL, NULL, m_wAchievementsProgress );
		}
	}
}
