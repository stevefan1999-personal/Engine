...
void CUIGameData::ExecuteOverlayCommand( char const *szCommand )
{
	...
	{
		DisplayOkOnlyMsgBox( NULL, "#GameUI_SteamOverlay_Title", "#GameUI_SteamOverlay_Text" );
	}
#else
...
bool CUIGameData::CheckAndDisplayErrorIfOffline( CBaseModFrame *pCallerFrame, char const *szMsg )
{
	...
	DisplayOkOnlyMsgBox( pCallerFrame, "#GameUI_XboxLive", szMsg );
	return true;
#endif
...
bool CUIGameData::CheckAndDisplayErrorIfNotSignedInToLive( CBaseModFrame *pCallerFrame )
{
	...
	{
		szMsg = "#GameUI_MsgBx_NeedLiveNonGoldMsg";
		...
		{
			szMsg = "#GameUI_MsgBx_NeedLiveNonGoldSplitscreenMsg";
		}
#endif
	}
	else
	{
		szMsg = "#GameUI_MsgBx_NeedLiveSinglescreenMsg";

#ifdef _X360
		// Show the splitscreen version if there are 2 non-guest accounts
		if ( XBX_GetNumGameUsers() > 1 && XBX_GetUserIsGuest( 0 ) == false && XBX_GetUserIsGuest( 1 ) == false )
		{
			szMsg = "#GameUI_MsgBx_NeedLiveSplitscreenMsg";
		}
#endif
	}
...
void CUIGameData::ShowPasswordUI( char const *pchCurrentPW )
{
	...
	if ( pwEntry )
	{
		...
		data.pWindowTitle = "#GameUI_PasswordEntry_Title";
		data.pMessageText = "#GameUI_PasswordEntry_Prompt";
		data.bOkButtonEnabled = true;
...
void CUIGameData::OnEvent( KeyValues *pEvent )
{
	...
	else if ( !Q_stricmp( "OnProfileUnavailable", szEvent ) )
	{
		...
		data.pWindowTitle = "#GameUI_MsgBx_AchievementNotWrittenTitle";
		data.pMessageText = "#GameUI_MsgBx_AchievementNotWritten"; 
		data.bOkButtonEnabled = true;
	...
	else if ( !Q_stricmp( "OnInvite", szEvent ) )
	{
		...
		else if ( !Q_stricmp( "error", pEvent->GetString( "action" ) ) )
		{
			...
			data.pWindowTitle = "#GameUI_XboxLive";
	...
	else if ( !Q_stricmp( "OnSysStorageDevicesChanged", szEvent ) )
	{
		...
		data.pMessageText = "#GameUI_MsgBx_StorageDeviceRemoved"; 
		data.bOkButtonEnabled = true;
	...
	else if ( !Q_stricmp( "OnSysInputDevicesChanged", szEvent ) )
	{
		...
		if ( CBaseModPanel::GetSingleton().IsVisible() || nInactivePlayers == XBX_GetNumGameUsers() )
		{
			...
			GenericConfirmation::Data_t data;
			data.pWindowTitle = "#GameUI_MsgBx_ControllerUnpluggedTitle";
			data.pMessageText = "#GameUI_MsgBx_ControllerUnplugged";
			data.bOkButtonEnabled = true;
	...
	else if ( !Q_stricmp( "OnMatchPlayerMgrReset", szEvent ) )
	{
		...
		if ( confirmation )
		{
			...
			data.pWindowTitle = "#GameUI_MsgBx_SignInChangeC";
			data.pMessageText = "#GameUI_MsgBx_SignInChange";
			data.bOkButtonEnabled = true;

			if ( !Q_stricmp( szReason, "GuestSignedIn" ) )
			{
				data.pWindowTitle = "#GameUI_MsgBx_DisconnectedFromSession";	// "Disconnect"
				data.pMessageText = "#GameUI_MsgBx_SignInChange";				// "Sign-in change has occured."
			}
	...
	else if ( !Q_stricmp( "OnEngineDisconnectReason", szEvent ) )
	{
		...
		RemapText_t arrText[] = {
			...
			{ "Connection to server timed out", "#GameUI_MsgBx_DisconnectedFromServer", RemapText_t::MATCH_SUBSTR },
	...
	else if ( !Q_stricmp( "OnMatchSessionUpdate", szEvent ) )
	{
		if ( !Q_stricmp( "error", pEvent->GetString( "state", "" ) ) )
		{
			...
			char const *szErrorTitle = "#GameUI_MsgBx_DisconnectedFromSession";
