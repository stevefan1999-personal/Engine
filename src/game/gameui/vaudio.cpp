...
void Audio::Activate()
{
	...
	else
	{
		...
		if ( m_drpVoiceCommunicationStyle )
		{
			...
			{
				m_drpVoiceCommunicationStyle->SetCurrentSelection( "#GameUI_OpenMic" );
				SetControlEnabled( "SldVoiceVoxThreshold", bVoiceEnabled );
			}
			else
			{
				m_drpVoiceCommunicationStyle->SetCurrentSelection( "#GameUI_PushToTalk" );
...
{
	//BaseClass::DrawDialogBackground( "#GameUI_Audio", NULL, "#GameUI_AudioVideo_Desc", NULL, NULL, true );
}
