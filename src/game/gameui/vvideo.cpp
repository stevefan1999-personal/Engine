...
void PagedPoolMemOpenned( DropDownMenu *pDropDownMenu, FlyoutMenu *pFlyoutMenu )
{
	...
	data.pWindowTitle = "#GameUI_VideoOptions_Paged_Pool_Mem";
	data.pMessageText = "#GameUI_VideoOptions_Paged_Pool_Mem_Info";
...
void Video::Activate( bool bRecommendedSettings )
{
	...
	if ( m_drpDisplayMode )
	{
		if ( m_bWindowed )
		{
			if ( m_bNoBorder )
			{
				m_drpDisplayMode->SetCurrentSelection( "#GameUI_VideoOptions_Windowed_NoBorder" );
			}
			else
			{
	...
	if ( m_drpLockMouse )
	{
		if ( m_bLockMouse )
		{
			m_drpLockMouse->SetCurrentSelection( "#GameUI_Enabled" );
		}
		else
		{
			m_drpLockMouse->SetCurrentSelection( "#GameUI_Disabled" );
		}
...
void Video::OnCommand(const char *command)
{
	...
	else if ( !Q_strcmp( command, "#GameUI_VideoOptions_Windowed_NoBorder" ) )
	{
		m_bWindowed = true;
		m_bNoBorder = true;
		m_bDirtyValues = true;
...
void Video::PaintBackground()
{
	...
	//BaseClass::DrawDialogBackground( bIsAdvanced ? "#GameUI_VideoAdvanced_Title" : "#GameUI_Video", NULL, "#GameUI_AudioVideo_Desc", NULL, NULL, true );
}
