...
void LeaderboardListItem::OnCommand( const char *command )
{
	...
	else if ( !Q_strcmp( command, "#GameUI_SendMessage" ) )
	{
	...
	else if ( !Q_strcmp( command, "#GameUI_ViewSteamID" ) )
	{
		CBaseModPanel::GetSingleton().PlayUISound( UISOUND_ACCEPT );
		char steamCmd[64];
...
void Leaderboard::Activate()
{
	...
	if ( CBaseModFooterPanel *footer = BaseModUI::CBaseModPanel::GetSingleton().GetFooterPanel() )
		footer->SetButtonText( FB_XBUTTON, "#GameUI_Leaderboard_Show_Global" );
...
void Leaderboard::OnThink()
{
	...
	if ( pNoEntriesLabel )
	{
		...
		if ( bVisible )
		{
			char const *szText = "#GameUI_Leaderboard_No_Times";

			if ( !AnyUserConnectedToLIVE() )
				szText = "#GameUI_MainMenu_SurvivalLeaderboards_Tip_Disabled";
			else if ( IsAQueryPending() )
				szText = "#GameUI_StillSearching";
...
void Leaderboard::CmdToggleLeaderboardType()
{
	...
	if( footer )
	{
		...
		default:
			footer->SetButtonText( FB_XBUTTON, "#GameUI_Leaderboard_Show_Friends" );
			break;
		}
	}
...
void Leaderboard::PaintBackground()
{
	...
	if ( m_Mode == LEADERBOARD_FRIENDS )
		pszDescription = "#GameUI_Leaderboard_Subtitle_Survival_Friends";
	else
		pszDescription = "#GameUI_Leaderboard_Subtitle_Survival_Global";

	BaseClass::DrawDialogBackground( "#GameUI_Leaderboard_Title", NULL, pszDescription, NULL, NULL, false, m_iTitleXOffset );
}
...
void Leaderboard::UpdateFooter()
{
	...
	if ( footer )
	{
		...
		footer->SetButtonText( FB_BBUTTON, "#GameUI_Back" );
		footer->SetButtonText( FB_ABUTTON, "#GameUI_ViewGamerCard" );
		footer->SetButtonText( FB_YBUTTON, "#GameUI_Leaderboard_Join_Game" );
	}
#endif
}
