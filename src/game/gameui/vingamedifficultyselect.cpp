...
void InGameDifficultySelect::LoadLayout()
{
	...
	if ( z_difficulty.IsValid() )
	{
		...
		char chBuffer[64];
		Q_snprintf( chBuffer, ARRAYSIZE( chBuffer ), "#GameUI_Difficulty_%s", z_difficulty.GetString() );
		...
		if ( pwcDifficulty )
		{
			...
			g_pVGuiLocalize->ConstructString( szWideBuff, sizeof( szWideBuff ), g_pVGuiLocalize->Find( "#GameUI_GameSettings_Current_Difficulty" ), 1, pwcDifficulty );
