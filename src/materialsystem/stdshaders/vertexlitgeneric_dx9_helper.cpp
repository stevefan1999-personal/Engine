	...
	if ( pShader->IsSnapshotting() || (! pContextData ) || ( pContextData->m_bMaterialVarsChanged ) )
	{
		...
		//	bool bHasLegacyEnvSphereMap = bHasEnvmap && IS_FLAG_SET(MATERIAL_VAR_ENVMAPSPHERE);
			...
			if ( bHasBaseTexture )
			{
				...
				// If we're on macOS GL on a crappy OS which can't do sRGB from render targets, check to see if we're reading from one...
				// change from IsOSX() to IsMac()
				/*if ( IsMac() && !g_pHardwareConfig->CanDoSRGBReadFromRTs() )
				{
